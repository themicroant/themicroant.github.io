...
const Sound = (() => {
    ...
    let muted = false;
    try {
        muted = localStorage.getItem('stt-muted') === '1';
    }
    catch (e) { /* ignore */ }
    function context() {
        if (!ctx) {
    ...
    function unlock() { const c = context(); if (c && c.state === 'suspended')
        c.resume(); }
    ...
    return {
        unlock,
        toggleMute() { muted = !muted; try {
            localStorage.setItem('stt-muted', muted ? '1' : '0');
        }
...
function buildDeck(recipe) {
    const deck = [];
    recipe.forEach(([count, key]) => { for (let i = 0; i < count; i++)
        deck.push(makeCard(key)); });
    return deck;
}
...
const EVENTS = [
    ...
    { text: '⚰️ A dusty sarcophagus stands ajar. Something glints inside.',
        choices: [
            { label: '🏺 Take the relic (random relic)', effect: p => { const key = pickUnownedRelic(p); if (key) {
                    p.relics.push(key);
                    return `You claim ${RELIC_LIBRARY[key].name} ${RELIC_LIBRARY[key].emoji}!`;
                } p.gold += 50; return 'No relics left to find; you take 50 gold instead.'; } },
    ...
    { text: '🧝 A wandering healer offers to mend your wounds — for a price.',
        choices: [
            { label: '💗 Pay 30 Gold to heal fully', effect: p => { if (p.gold >= 30) {
                    p.gold -= 30;
                    p.hp = p.maxHp;
    ...
    { text: '🃏 A gambler challenges you to a game of chance.',
        choices: [
            { label: '🎲 Gamble 25 Gold', effect: p => { if (p.gold < 25)
                    return 'You cannot afford to gamble.'; p.gold -= 25; if (Math.random() < 0.5) {
                    p.gold += 60;
                    return 'You win! +60 gold.';
                } return 'You lose the bet.'; } },
...
function buildMap() {
    ...
    for (let r = 0; r < TOTAL_ROWS; r++) {
        ...
        const row = [];
        for (let c = 0; c < n; c++)
            row.push({ row: r, col: c, type: null, visited: false });
        rows.push(row);
    }
    // Assign node types
    ...
    for (let r = 1; r < TOTAL_ROWS - 2; r++) {
        rows[r].forEach(node => {
            ...
            const bag = [];
            options.forEach(t => { for (let i = 0; i < weights[t]; i++)
                bag.push(t); });
    ...
    const edges = {}; // key `${r}-${c}` -> target col indices in row r+1
    for (let r = 0; r < TOTAL_ROWS - 1; r++) {
        ...
        for (let c = 0; c < fromCount; c++) {
            ...
            if (!candidates.length)
                candidates.push(Math.min(c, toCount - 1));
            edges[`${r}-${c}`] = candidates;
        }
        // ensure incoming coverage
        const incoming = new Set();
        for (let c = 0; c < fromCount; c++)
            edges[`${r}-${c}`].forEach(t => incoming.add(t));
...
function enemyThreat(data, floor) {
    ...
    data.pattern.forEach(moveKey => {
        ...
        if (mv.kind === 'buff' && mv.strength)
            strengthPerCycle += mv.strength;
    });
...
function difficultyTuning(nodeType, floor) {
    if (nodeType === 'elite')
        return { ideal: 0.30 + floor * 0.006, cap: 0.60, variety: 2 };
...
function logCombat(msg) {
    ...
    if (state.combat.log.length > 40)
        state.combat.log.length = 40;
}
function drawCards(n) {
    const c = state.combat;
    for (let i = 0; i < n; i++) {
        if (!c.drawPile.length) {
            if (!c.discardPile.length)
                return;
...
function startPlayerTurn() {
    ...
    if (c.turn === 1 && hasRelic(p, 'manaCrystal'))
        c.energy += 1;
    const drawAmt = 5 + (hasRelic(p, 'ringSnake') ? 1 : 0);
    drawCards(drawAmt);
    if (hasRelic(p, 'regenCharm'))
...
function playCard(card, targetEnemy) {
    ...
    if (data.loseHp)
        p.hp = Math.max(1, p.hp - data.loseHp);
    if (data.exhaust)
...
function usePotion(key, targetEnemy) {
    const c = state.combat;
    const p = state.player;
    if (!p.potions.includes(key))
        return;
    const data = POTION_LIBRARY[key];
    if (data.target && !targetEnemy) {
        state.pendingPotion = key;
        render();
        return;
    }
...
function enemyTurn() {
    ...
    for (const enemy of c.enemies) {
        ...
        if (move.kind === 'attack') {
            ...
            if (dealt > 0) {
                c.fx.push({ kind: 'playerDmg', n: dealt });
                Sound.hurt();
            }
            ...
            const gain = move.strength ?? 0;
            enemy.strength += gain;
            logCombat(`💪 ${enemy.name} gains ${gain} Strength.`);
        }
            ...
            if (move.weak) {
                c.playerWeak += move.weak;
                logCombat(`⬇️ You are Weak (${c.playerWeak}).`);
            }
            if (move.vulnerable) {
                c.playerVulnerable += move.vulnerable;
                logCombat(`☠️ You are Vulnerable (${c.playerVulnerable}).`);
            }
        }
...
function resolveCombatVictory() {
    ...
    if (hasRelic(p, 'goldenIdol'))
        goldWon = Math.round(goldWon * 1.25);
    p.gold += goldWon;
    let relicWon = null;
...
function enterShop() {
    ...
    const relicKeys = [];
    for (let i = 0; i < 2; i++) {
        const k = pickUnownedRelic({ relics: state.player.relics.concat(relicKeys) });
        if (k)
            relicKeys.push(k);
    }
...
function buyCard(idx) {
    ...
    if (!item || item.bought || p.gold < item.price)
        return;
    p.gold -= item.price;
...
function buyPotion(idx) {
    ...
    if (!item || item.bought || p.gold < item.price || p.potions.length >= p.maxPotions)
        return;
    p.gold -= item.price;
...
function removeCardFromDeck(cardUid) {
    ...
    if (sd.removeUsed || p.gold < sd.removePrice)
        return;
...
function renderTopbar() {
    if (state.screen === 'menu')
        return;
    const p = state.player;
    el('tb-floor').textContent = String(state.floorsClimbed);
    el('tb-hp').textContent = String(p.hp);
    el('tb-maxhp').textContent = String(p.maxHp);
    el('tb-gold').textContent = String(p.gold);
...
function renderMap() {
    ...
    state.map.rows.forEach((row, r) => {
        ...
        row.forEach(node => {
            ...
            if (isNext)
                cls += ' reachable';
            if (!isNext && !isCurrent && !node.visited)
                cls += ' faded';
            btn.className = cls;
            btn.textContent = (node.type ? NODE_ICON[node.type] : '') || '❔';
            btn.title = node.type || '';
            if (isNext)
                btn.onclick = () => enterNode(r, node.col);
...
function renderCombat() {
    ...
    c.enemies.forEach(en => {
        ...
        const statuses = [];
        if (en.strength)
            statuses.push(`💪${en.strength}`);
        if (en.vulnerable)
            statuses.push(`☠️${en.vulnerable}`);
        if (en.weak)
            statuses.push(`⬇️${en.weak}`);
        div.innerHTML = `
    ...
    `;
        if (en.hp > 0) {
            if (needsTarget)
                div.onclick = () => playCard(selected, en);
            else if (pendingPotion)
                div.onclick = () => usePotion(pendingPotion, en);
        }
  ...
  `;
    ...
    p.potions.forEach((key, i) => {
        ...
        btn.onclick = () => {
            if (data.target) {
                state.pendingPotion = state.pendingPotion === key ? null : key;
                render();
            }
            else
                usePotion(key, null);
...
function renderShop() {
    ...
    sd.relicStock.forEach((item, idx) => {
        ...
        badge.style.opacity = String(item.bought || p.gold < item.price ? .5 : 1);
    ...
    sd.potionStock.forEach((item, idx) => {
        ...
        badge.className = 'relic-badge';
        badge.style.cursor = item.bought ? 'default' : 'pointer';
        badge.style.opacity = String(item.bought || p.gold < item.price || p.potions.length >= p.maxPotions ? .5 : 1);
...
el('deck-overlay').onclick = (e) => { if (e.target === el('deck-overlay'))
    toggleDeckOverlay(); };
el('btn-mute').onclick = () => { const muted = Sound.toggleMute(); if (!muted) {
    Sound.unlock();
    Sound.click();
} render(); };
...
window.addEventListener('resize', () => { if (state && state.screen === 'map')
    renderMap(); });
