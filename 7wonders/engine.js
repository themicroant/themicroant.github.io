...
//
// Scope: the base game for 3 to 7 players (docs/requirements.md §1). Not implemented: the
...
//
// Types (Game, Player, Card, etc.) come from src/types.d.ts, visible here as global ambient
// declarations with no import — see that file's header for why.
"use strict";
...
function playersInScope(game, playerIdx, scope) {
    ...
    if (scope === "neighbors")
        return [left, right];
    if (scope === "neighborsAndSelf" || scope === "selfAndNeighbors")
        return [playerIdx, left, right];
...
// ---- setup / dealing ----
// Deck composition per Age: filters GD.CARDS down to what's valid at this player count
// (`playerCount <= numPlayers` — the base pool plus whichever "extra content" tiers this count
// unlocks, see game-data.ts's CARDS/SUPPORTED_PLAYER_COUNTS comments), then deals 7×numPlayers
// of it. Age III follows the real rule of keeping `numPlayers + 2` random Guilds and shuffling
// them back in among the non-Guild Age III cards (docs/rules.md §Setup).
function buildDeckPools(numPlayers) {
    ...
    Object.entries(pools).forEach(([age, pool]) => {
        if (pool.length < handTotal) {
            throw new Error(`Age ${age} card pool holds ${pool.length} cards but ${numPlayers} players need ${handTotal}`);
        }
    });
    return pools;
}
...
function tradeUnitCost(game, playerIdx, resource) {
    ...
    // stage is built.
    if (category === "basic" && hasPower(p, "cheapRawTrade"))
        return 1;
...
function applyPurchases(game, playerIdx, purchases) {
    ...
    purchases.forEach(({ resource, count, from, unitCost }) => {
        ...
        game.log.push(`${game.players[playerIdx].name} buys ${count} ${resource} from ${game.players[sellerIdx].name} for ${total} coins.`);
    });
}
...
function applyWonderStage(game, playerIdx, cardId) {
    ...
    if (p.wonderStagesBuilt >= p.wonder.stages.length)
        return { success: false, reason: "wonder-complete" };
    ...
    if (stage.effect.power === "discardPileBuild")
        p.discardPileBuildsAvailable += 1;
    game.log.push(`${p.name} builds Wonder stage ${p.wonderStagesBuilt} of ${p.wonder.name} (side ${p.wonder.side}).`);
    return { success: true };
}
...
function applyFreeBuildFromHand(game, playerIdx, cardId) {
    const p = game.players[playerIdx];
    if (!hasPower(p, "freeBuildPerAge") || p.freeBuildUsedThisAge)
...
function applyAction(game, playerIdx, action, cardId) {
    if (action === "build")
        return applyBuild(game, playerIdx, cardId);
    if (action === "wonder")
        return applyWonderStage(game, playerIdx, cardId);
    if (action === "discard")
        return applyDiscard(game, playerIdx, cardId);
    if (action === "freeBuild")
        return applyFreeBuildFromHand(game, playerIdx, cardId);
    if (action === "discardPileBuild")
        return applyDiscardPileBuild(game, playerIdx, cardId);
...
function getAvailableActionsForCard(game, playerIdx, cardId) {
    ...
    const wonderAfford = wonderNextStage
...
function aiChooseAction(game, playerIdx) {
    ...
    if (!p.hand.length)
        return null;
    let best = null;
    const consider = (candidate) => { if (!best || candidate.value > best.value)
        best = candidate; };
    p.hand.forEach((cardId) => {
        ...
        const afford = canAffordWithCommerce(game, playerIdx, cost);
        if (afford.ok)
            consider({ action: "build", cardId, value: cardHeuristicValue(card) - (afford.coinsNeeded || 0) * 0.5 });
    });
    if (p.wonderStagesBuilt < p.wonder.stages.length) {
        const stage = p.wonder.stages[p.wonderStagesBuilt];
        const afford = canAffordWithCommerce(game, playerIdx, stage.cost);
    ...
    if (!best) {
        ...
        return { action: "discard", cardId: worst, value: 0 };
    }
    return best;
}
function aiUseBonusPowers(game, playerIdx) {
    ...
    while (p.discardPileBuildsAvailable > 0) {
        ...
        if (cardHeuristicValue(CARD_BY_ID[best]) <= 3)
            break;
        if (!applyDiscardPileBuild(game, playerIdx, best).success)
            break;
    }
}
function runAiTurns(game) {
    for (let i = 1; i < game.numPlayers; i++) {
        ...
        if (choice)
            applyAction(game, i, choice.action, choice.cardId);
    }
}
// ---- military ----
...
function resolveMilitary(game) {
    ...
    game.players.forEach((p, idx) => {
        ...
        [left, right].forEach((nIdx) => {
            if (strengths[idx] > strengths[nIdx])
                p.militaryTokens.push(tokenValue);
            else if (strengths[idx] < strengths[nIdx])
                p.militaryTokens.push(-1);
        });
    });
...
function resolveLeftoverCard(game, playerIdx) {
    ...
    if (hasPower(p, "lastCardAlternative")) {
        ...
        if (!worthBuilding || !applyBuild(game, playerIdx, cardId).success) {
            if (!applyWonderStage(game, playerIdx, cardId).success)
                applyDiscard(game, playerIdx, cardId);
        }
    }
    else {
        game.discardPile.push(cardId);
...
function countCardType(game, playerIdx, { cardType, scope }) {
    return playersInScope(game, playerIdx, scope).reduce((count, idx) => count + game.players[idx].built.filter((id) => CARD_BY_ID[id].type === cardType).length, 0);
}
function countCardTypes(game, playerIdx, { cardTypes, scope }) {
    return playersInScope(game, playerIdx, scope).reduce((count, idx) => count + game.players[idx].built.filter((id) => cardTypes.includes(CARD_BY_ID[id].type)).length, 0);
}
...
function countDefeatTokens(game, playerIdx, scope) {
    return playersInScope(game, playerIdx, scope).reduce((sum, idx) => sum + game.players[idx].militaryTokens.filter((t) => t < 0).length, 0);
}
...
function computeGuildScore(game, playerIdx) {
    ...
    if (hasPower(p, "copyNeighborGuild")) {
        ...
        if (bestName) {
            total += best;
            details.push({ card: `${bestName} (copied)`, vp: best });
        }
    }
