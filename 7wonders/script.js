...
//
// Types (Game, Card, UiState, etc.) come from src/types.d.ts, visible here as global ambient
...
function wonderStageEffectHtml(effect) {
    const bits = [];
    if (effect.vp)
        bits.push(`${effect.vp} ${iconImg(GameData.ICONS.vp, "victory points")}`);
    if (effect.coins)
        bits.push(`+${effect.coins} ${iconImg(GameData.ICONS.coins, "coins")}`);
    if (effect.shields)
        bits.push(`${effect.shields} ${iconImg(GameData.ICONS.shields, "shields")}`);
...
function renderSetup(app) {
  ...
  `;
    ...
    document.getElementById("wonder-detail-modal")?.addEventListener("click", (e) => {
        if (e.target.id === "wonder-detail-modal")
            closeDetail();
    });
    document.getElementById("start-btn").addEventListener("click", () => {
        if (!state.setup.wonderId)
            return;
...
// `game` isn't read — kept for symmetry with the other wire*/render* functions' signatures and
// because call sites always have it on hand. Pre-existing in the original JS; underscore-prefixed
// so strict TS's noUnusedParameters (correctly) doesn't flag it.
function wireActionBar(_game) {
...
function renderHand(app, opts = {}) {
  ...
  `;
    ...
    hand.addEventListener("keydown", (e) => {
        ...
        let nextIndex = null;
        if (e.key === "ArrowRight")
            nextIndex = Math.min(cards.length - 1, Math.max(0, currentIndex) + 1);
        else if (e.key === "ArrowLeft")
            nextIndex = Math.max(0, currentIndex - 1);
        else if (e.key === "ArrowDown")
            nextIndex = Math.min(cards.length - 1, Math.max(0, currentIndex) + cols);
...
function chooseAction(action) {
    ...
    if (!result.success)
        return; // action buttons are disabled when unavailable; this is a safety no-op
    state.selectedCardId = null;
    state.freeBuildArmed = false;
    if ("militarySummary" in result) {
...
function render(opts = {}) {
    ...
    if (state.screen === "hand")
        return renderHand(app, opts);
    if (state.screen === "ageEnd")
        return renderAgeEnd(app);
    if (state.screen === "gameEnd")
        return renderGameEnd(app);
}
render();
